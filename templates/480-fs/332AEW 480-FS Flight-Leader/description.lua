livery = {

-- F16_bl50_Kil	
	{"F16_bl50_Kil", 0 ,"F16_bl50_Kil",false};
	{"F16_bl50_Kil", 13 ,"F16_bl50_Kil_RoughMet",false};
	
	
-- F16_bl50_Main_1	
	{"F16_bl50_Main_1", 0 ,"F16_bl50_Main_1",false};
	{"F16_bl50_Main_1", NORMAL_MAP ,"F16_bl50_Main_1_Normal",false};
	{"F16_bl50_Main_1", 13 ,"F16_bl50_Main_1_RoughMet",false};
	
	
-- F16_bl50_Main_2	
	{"F16_bl50_Main_2", 0 ,"F16_bl50_Main_2",false};
	{"F16_bl50_Main_2", 13 ,"F16_bl50_Main_2_RoughMet",false};
	
	
-- F16_bl50_Main_3
	{"F16_bl50_Main_3", 0 ,"F16_bl50_Main_3",false};
	{"F16_bl50_Main_3", 13 ,"F16_bl50_Main_3_RoughMet",false};
	
	
-- F16_bl50_wing_L
	{"F16_bl50_wing_L", 0 ,"F16_bl50_wing_L",false};
	{"F16_bl50_wing_L", 13 ,"F16_bl50_wing_L_RoughMet",false};
	
	
-- F16_bl50_wing_R
	{"F16_bl50_wing_R", 0 ,"F16_bl50_wing_R",false};
	{"F16_bl50_wing_R", 13 ,"F16_bl50_wing_R_RoughMet",false};
	
-- F16_bl50_wing_Pylon_1
	{"F16_bl50_Wing_Pylon_1", 0 , "F16_bl50_Wing_Pylon_1",false};
	
	
-- F16_bl50_wing_Pylon_2
	{"F16_bl50_Wing_Pylon_2", 0 , "F16_bl50_Wing_Pylon_2",false};
	
	
-- F16_bl50_LAU_129	
	{"LAU_129", 0 , "LAU_129_diff", false};
	
-- Patch
	{"pilot_F16_patch", 0 ,"pilot_F16_patch",false};
	{"pilot_F16_patch", 13 ,"pilot_F16_patch_RoughMet",false};
	{"pilot_F16_patch", NORMAL_MAP ,"pilot_F16_patch_nm",false};
	
-- Helmet	
	{"pilot_F16_helmet",  0 ,"pilot_f16_helmet", false};
	{"pilot_F16_helmet",  13 ,"pilot_f16_helmet_roughmet", false};
	
-- Visor
	{"pilot_F16_helmet_glass",  0 ,"pilot_f16_visor", false};
	{"pilot_F16_helmet_glass",  13 ,"pilot_f16_visor_roughmet", false};

--F16_bl50_FIN_DECAL	
	{"F16_bl50_FIN_DECAL",	0,	   "f16_bl50_Kil",			false};
	{"F16_bl50_FIN_DECAL",	1,	   "f16_bl50_Kil_normal",	true};
	{"F16_bl50_FIN_DECAL",	13,	   "f16_bl50_Kil_RoughMet",	false};
	{"F16_bl50_FIN_DECAL",	DECAL, "F16_AF_96_Decal",		false};
	
--F16_bl50_FIN_BORT_NUMBER
	{"F16_bl50_FIN_BORT_NUMBER",	0,	    "f16_bl50_Kil",			 false};
	{"F16_bl50_FIN_BORT_NUMBER",	1,	    "f16_bl50_Kil_normal",	 true};
	{"F16_bl50_FIN_BORT_NUMBER",	13,		"f16_bl50_Kil_RoughMet", false};
	{"F16_bl50_FIN_BORT_NUMBER",	DECAL,	"F16_bort_number",		 false};
	
	
--F16_bl50_NOUSE_DECAL
	{"F16_bl50_MAT1_BORT_NUMBER_X100",	0,	    "f16_bl50_Main_1",			false};
	{"F16_bl50_MAT1_BORT_NUMBER_X100",	1,	    "f16_bl50_Main_1_normal",	false};
	{"F16_bl50_MAT1_BORT_NUMBER_X100",	13,  	"f16_bl50_Main_1_RoughMet", false};
	{"F16_bl50_MAT1_BORT_NUMBER_X100",	DECAL,	"F16_bort_number", 			false};
	
--F16_bl50_REFUEL_DECAL 
	{"F16_bl50_MAT1_BORT_NUMBER_DECAL",	0,	    "f16_bl50_Main_1",			false};
	{"F16_bl50_MAT1_BORT_NUMBER_DECAL",	1,	    "f16_bl50_Main_1_normal",	false};
	{"F16_bl50_MAT1_BORT_NUMBER_DECAL",	13,	    "f16_bl50_Main_1_RoughMet", false};
	{"F16_bl50_MAT1_BORT_NUMBER_DECAL",	DECAL,	"F16_AF_96_Decal",			false};
	
--F16_bl50_NOUSE AND REFUEL BORT_NUMBER
	{"F16_bl50_MAT1_BORT_NUMBER",	0,	    "f16_bl50_Main_1",			false};
	{"F16_bl50_MAT1_BORT_NUMBER",	1,	    "f16_bl50_Main_1_normal",	false};
	{"F16_bl50_MAT1_BORT_NUMBER",	13,		"f16_bl50_Main_1_RoughMet", false};
	{"F16_bl50_MAT1_BORT_NUMBER",	DECAL,	"F16_bort_number", 			false};
	
--Engine
	{"F16_bl50_Engine", 0 ,"F16_bl50_Engine",false};
	{"F16_bl50_Engine", 13 ,"F16_bl50_Engine_RoughMet",false};
}

custom_args =
{
[1000] = 0.0, 
[1001] = 1.0, 
[1002] = 1.0, 
[1003] = 1.0, 
}

name = "332AEW 480-FS Flight-Leader"
countries = {"USA"}

--Made by extho